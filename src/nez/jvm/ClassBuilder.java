...
public class ClassBuilder extends ClassWriter implements Opcodes {
	 ...
	 * @param interfaces
	 */
	public ClassBuilder(String fullyQualifiedClassName, String sourceName, Class<?> superClass,
			Class<?>[] interfaces) {
	 ...
	 */
	public ClassBuilder(int accessFlag, String fullyQualifiedClassName, String sourceName,
		...
		this.visit(V1_7, accessFlag, this.internalName, null, Type.getInternalName(superClass),
				interfaceNames);
	...
	public static class MethodBuilder extends GeneratorAdapter {
		...
		protected MethodBuilder(int accessFlag, Method method, ClassVisitor cv) {
			super(Opcodes.ASM4, toMethodVisitor(accessFlag, method, cv), accessFlag, method
					.getName(), method.getDescriptor());
		...
		private static MethodVisitor toMethodVisitor(int access, Method method, ClassVisitor cv) {
			MethodVisitor visitor = cv.visitMethod(access, method.getName(),
					method.getDescriptor(), null, null);
			JSRInlinerAdapter inlinerAdapter = new JSRInlinerAdapter(visitor, access,
		 ...
		 * 
		 * @return
		 *         - stack of label pair. pair's left value is break label,
	...
	private static class VarScopes extends ArrayDeque<LocalVarScope> {
		 ...
		 * if this builder represents static method or static initializer, index
		 * = 0.
	...
	public static class VarEntry {
		 ...
		 * 
		 * @return
		 *         return null if this entry represents return address
		 */
